...
public class WindowOverlay : ThreadedServiceBase
{
    ...
    {
        // Verifica a posição da janela do jogo apenas a cada 500ms 
        // ou se for a primeira execução. Isso poupa MUITO processamento.
        if (DateTime.Now < _nextCheck) return;
        _nextCheck = DateTime.Now.AddMilliseconds(500);

        // Só atualiza se houver mudança real de posição/tamanho
        ...
        // Se precisar usar Invoke, use BeginInvoke (Assíncrono) para não travar a thread de leitura
        ...
        }, DispatcherPriority.Render); // Prioridade de Render é maior que Normal
    }
