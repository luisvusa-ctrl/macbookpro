...
public class Graphics : ThreadedServiceBase
{
    ...
    private void InitializeDevice()
    {
        var parameters = new PresentParameters
        {
            ...
            PresentationInterval = PresentInterval.Immediate, // VSync OFF para fluidez
    ...
    private void RenderFrame()
    {
        ...
        lock (_deviceLock)
        {
            ...
            // Sincroniza a matriz imediatamente antes do desenho para evitar o "atraso" visual
    ...
    private void DrawPlayerEsp(ConfigManager features)
    {
        ...
        // Leitura direta da posição para evitar conflitos de proteção de membros
        ...
        foreach (var entity in GameData.Entities)
        {
            ...
            if (features.DrawWeaponText || features.DrawWeaponIcon) EspWeapon.Draw(this, entity, topLeft, bottomRight, features);
    ...
    // RESOLVE SKELETON: Método para desenhar linhas individuais entre ossos
    ...
    public void DrawThickLine(Color color, Vector2 start, Vector2 end, float thickness)
{
    ...
    Vertex[] vertices = new Vertex[4];
    // Criando um retângulo perfeito entre os dois pontos
    ...
    // CONFIGURAÇÃO CRÍTICA DO ESTADO DO DISPOSITIVO
    _device.SetRenderState(RenderState.AlphaBlendEnable, true);
    _device.SetRenderState(RenderState.CullMode, Cull.None); // Garante que a linha apareça dos dois lados
    _device.VertexFormat = VertexFormat.PositionRhw | VertexFormat.Diffuse;
