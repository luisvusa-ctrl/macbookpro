...
public class Graphics : ThreadedServiceBase
{
    ...
    private void InitializeDevice()
    {
        ...
        BombTime = new Font(_device, new FontDescription
        {
            Height = 30,
            FaceName = "Tahoma",
            Weight = FontWeight.Bold,
    ...
    private void DrawPlayerEsp(ConfigManager features)
    {
        ...
        foreach (var entity in GameData.Entities)
        {
            ...
            if (features.DrawWeaponText || features.DrawWeaponIcon)
                EspWeapon.Draw(this, entity, topLeft, bottomRight, features);

            if (features.EspSound)
            {
                // Remova a linha do footPos, não é mais necessária para o efeito 3D
                EspSound.Draw(this, entity, localPos);
            }

            if (features.EspChams)
            {
                EspChams.Draw(this, entity);
            }

            if (features.SkeletonEsp) SkeletonEsp.Draw(this, entity);

            if (features.EspSnaplines) 
            {
                EspSnaplines.Draw(this, entity);
            }
        }
    }
    ...
    public void DrawFilledTriangle(Vector2 p1, Vector2 p2, Vector2 p3, Color color)
    {
        if (_device == null) return;
        var d3dColor = new ColorBGRA(color.R, color.G, color.B, color.A);
        ...
        vertices[0] = new Vertex { Position = new Vector4(p1.X, p1.Y, 0, 1.0f), Color = d3dColor };
        vertices[1] = new Vertex { Position = new Vector4(p2.X, p2.Y, 0, 1.0f), Color = d3dColor };
        vertices[2] = new Vertex { Position = new Vector4(p3.X, p3.Y, 0, 1.0f), Color = d3dColor };
        ...
        _device.DrawUserPrimitives(PrimitiveType.TriangleList, 1, vertices);
    }
    ...
    public SharpDX.Direct3D9.Device GetDevice()
    {
        return _device;
    }
    ...
    public void Draw3DTube(Vector3 startWorld, Vector3 endWorld, Color color, float baseThickness, Vector3 cameraPos)
    {
        if (_device == null || _line == null) return;

        Vector3 startScreen = _viewMatrix.Transform(startWorld);
        Vector3 endScreen = _viewMatrix.Transform(endWorld);

        if (startScreen.Z >= 1 || endScreen.Z >= 1) return;

        
        float distance = Vector3.Distance(cameraPos, (startWorld + endWorld) / 2f);
        float screenThickness = (baseThickness * 350f) / distance; 
        screenThickness = MathUtil.Clamp(screenThickness, 1f, 80f);

        _line.Width = screenThickness;
        _line.Begin();
        _line.Draw(new[] { new Vector2(startScreen.X, startScreen.Y), new Vector2(endScreen.X, endScreen.Y) }, color);
        _line.End();
        _line.Width = 2.0f;
    }
