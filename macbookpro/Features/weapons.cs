...
public class EspWeapon
{
    ...
    public static void Draw(CS2Cheat.Graphics.Graphics graphics, Entity entity, Vector2 position, Vector2 bottomRight, ConfigManager config)
    {
        ...
        {
            DrawWeaponTextAt(graphics, entity, new Vector2(position.X, currentY));
        ...
        if (config.DrawWeaponIcon)
        {
            DrawWeaponIconAt(graphics, entity, new Vector2(position.X, currentY));
        }
    }

    private static void DrawWeaponTextAt(CS2Cheat.Graphics.Graphics graphics, Entity entity, Vector2 position)
    {
        var font = graphics.Gamesense;
        if (font == null || string.IsNullOrEmpty(entity.CurrentWeaponName)) return;

        string nameStr = entity.CurrentWeaponName.ToUpper();
        var textSize = font.MeasureText(null, nameStr, FontDrawFlags.Left);
        int textWidth = textSize.Right - textSize.Left;

        int x = (int)(position.X - (textWidth / 2));
        ...
        for (int i = -1; i <= 1; i++)
        {
            ...
            {
                if (i == 0 && j == 0) continue;
                font.DrawText(null, nameStr, x + i, y + j, Color.Black);
            }
        }

        font.DrawText(null, nameStr, x, y, Color.White);
    }

    private static void DrawWeaponIconAt(CS2Cheat.Graphics.Graphics graphics, Entity entity, Vector2 position)
    {
        ...
        if (WeaponIcons.TryGetValue(entity.WeaponId, out var iconChar))
        {
            ...
            int iconX = (int)(position.X - (iconWidth / 2));
