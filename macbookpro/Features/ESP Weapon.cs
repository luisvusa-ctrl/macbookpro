    ...
    private static readonly Dictionary<int, string> WeaponIcons = new()
{
    ...
    public static void Draw(CS2Cheat.Graphics.Graphics graphics, Entity entity, Vector2 topLeft, Vector2 bottomRight)
    {
        ...
        // --- 1. DESENHAR O NOME (TEXTO COM BORDA GROSSA 8 DIREÇÕES) ---
        ...
        int textY = (int)(bottomRight.Y + 2f);

        // Borda grossa para o texto (Igual às suas Flags)
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
        ...
        // --- 2. DESENHAR O ÍCONE (BORDA SIMPLES/SOMBRA) ---
        if (WeaponIcons.TryGetValue(entity.WeaponId, out var iconChar))
        {
            ...
            // Ajustado para não sobrepor o texto grosso
            int iconY = textY + 9;

            // Apenas uma sombra simples de 1px para o ícone
